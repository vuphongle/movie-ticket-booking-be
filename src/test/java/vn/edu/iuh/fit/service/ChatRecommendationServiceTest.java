...
class ChatRecommendationServiceTest {
  ...
  @Mock private ChatCinemaLocator cinemaLocator;
  ...
  void extractJsonBlock_shouldFindJsonInsideCodeFence() {
    ...
    String json =
        ReflectionTestUtils.invokeMethod(chatRecommendationService, "extractJsonBlock", raw);
    assertThat(json)
        .isEqualTo("{\"userAge\": 18, \"companionAges\": [10], \"preferredGenres\": [\"ACTION\"]}");
  }
  ...
  void extractJsonBlock_shouldReturnEmptyObjectWhenNotFound() {
    String json =
        ReflectionTestUtils.invokeMethod(
            chatRecommendationService, "extractJsonBlock", "Không có JSON");
  ...
  void isLowQualityAnswer_shouldTriggerFallbackForApology() {
    RecommendedMovieResponse movie =
        RecommendedMovieResponse.builder().name("Titanic").ageRating(null).build();
  ...
  void findRelevantShowtimes_shouldRespectDateAndCinemaFilter() {
    ...
    Auditorium auditorium = Auditorium.builder().id(11).name("Phòng 1").cinema(cinema).build();
