...
/**
 * Test controller để debug SendPulse email
 * Endpoint này chỉ nên được sử dụng trong môi trường development
 */
@Slf4j
@RestController
...
public class TestEmailController {
   ...
   * GET /api/v1/test/send-email?to=recipient@example.com
   */
      ...
      String htmlBody = """
          ...
          <body style="font-family: Arial, sans-serif;">
            ...
            <p style="color: #666; font-size: 12px;">
              Email được gửi lúc: """ + java.time.LocalDateTime.now() + """
            </p>
          </body>
          </html>
          """;
  ...
  /**
   * Test template rendering và gửi email reset password
   * GET /api/v1/test/send-reset-password?to=recipient@example.com&username=TestUser
   */
  ...
  /**
   * Test template rendering only (không gửi email)
   * GET /api/v1/test/render-template?template=reset-password
   */
  ...
  public ResponseEntity<Map<String, Object>> testTemplateRendering(
    ...
    try {
      ...
      response.put("htmlPreview", htmlContent.substring(0, Math.min(200, htmlContent.length())) + "...");
  ...
  /**
   * Kiểm tra cấu hình SendPulse
   * GET /api/v1/test/sendpulse-config
   */
  ...
  public ResponseEntity<Map<String, Object>> checkConfig() {
    ...
    try {
      ...
      response.put("message", "SendPulse client is configured. Check logs for details when sending email.");
      response.put("endpoints", Map.of(
          "simple", "/api/v1/test/send-email?to=your@email.com",
          "resetPassword", "/api/v1/test/send-reset-password?to=your@email.com&username=YourName",
