...
import vn.edu.iuh.fit.config.payment.vnpay.VNPayConfig;
...
public class VNPayService {
    ...
    public String createOrder(int total, String orderInfo, String urlReturn, Integer expireSeconds) {
        Map<String, String> vnpParams = new HashMap<>();
        initializeOrderParams(vnpParams, total, orderInfo, urlReturn, expireSeconds);
        return buildPaymentUrl(vnpParams);
    }

    private void initializeOrderParams(
        ...
        log.info("Creating order with total: {}, orderInfo: {}, urlReturn: {}, expireSeconds: {}",
                total, orderInfo, urlReturn, expireSeconds);
        ...
        calendar.add(Calendar.SECOND, expireSeconds > 0 ? expireSeconds : 600);
    ...
    public int orderReturn(HttpServletRequest request) {
        ...
        try {
            ...
            while (parameterNames.hasMoreElements()) {
                ...
                if (fieldValue != null && !fieldValue.isEmpty()) {
                    fields.put(fieldName, fieldValue); // chưa encode
                }
            }
        } catch (Exception e) {
            log.error("Error reading VNPay request parameters: {}", e.getMessage());
        }
