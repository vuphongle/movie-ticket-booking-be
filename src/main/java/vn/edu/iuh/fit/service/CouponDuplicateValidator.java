 ...
 *
 * <p>This validator ensures that within a single coupon, there are no two CouponDetail entries that
 * have the exact same combination of: - targetType = TICKET - benefitType = DISCOUNT_PERCENT -
 * percent (normalized to 2 decimal places)
 *
 * <p>Other fields are ignored for simplicity after cleanup.
 */
@Slf4j
@Service
...
public class CouponDuplicateValidator {
   ...
   */
  public void validateNoDuplicateOrderDiscountPercent(
      Integer couponId, CouponDetail detail, Integer excludeDetailId) {
    // Only validate TICKET + DISCOUNT_PERCENT combinations
    if (detail.getTargetType() != TargetType.TICKET
        || detail.getBenefitType() != BenefitType.DISCOUNT_PERCENT) {
      return;
    }

    // Get all existing details for this coupon
    List<CouponDetail> existingDetails =
        couponDetailRepository.findByCouponIdOrderByIdAsc(couponId);
    ...
    for (CouponDetail existing : existingDetails) {
      ...
      if (existing.getTargetType() != TargetType.TICKET
          || existing.getBenefitType() != BenefitType.DISCOUNT_PERCENT) {
        continue;
      }
      ...
      if (isDuplicateOrderDiscountPercent(detail, existing)) {
        String message =
            "Không thể thêm điều kiện này vì đã tồn tại điều kiện giảm giá theo phần trăm tương tự cho vé.";
        log.warn(
            "Duplicate TICKET + DISCOUNT_PERCENT coupon detail detected. "
                + "Existing detail ID: {}, percent: {}",
            existing.getId(),
            existing.getTerms() != null
                ? normalizePercent(existing.getTerms().getPercent())
                : null);
  ...
  public void validateCouponNoDuplicateOrderDiscountPercent(Integer couponId) {
    ...
    List<CouponDetail> ticketDiscountDetails =
        allDetails.stream()
            .filter(
                detail ->
                    detail.getTargetType() == TargetType.TICKET
                        && detail.getBenefitType() == BenefitType.DISCOUNT_PERCENT)
            .toList();
    ...
    for (int i = 0; i < ticketDiscountDetails.size(); i++) {
      ...
      for (int j = i + 1; j < ticketDiscountDetails.size(); j++) {
        ...
        if (isDuplicateOrderDiscountPercent(detail1, detail2)) {
          String message =
              "Coupon không thể được kích hoạt vì có các điều kiện giảm giá theo phần trăm trùng lặp cho vé.";
          log.warn(
              "Duplicate TICKET + DISCOUNT_PERCENT coupon details found in coupon {}. "
                  + "Detail IDs: {} and {}, percent: {}",
              couponId,
              detail1.getId(),
              detail2.getId(),
              detail1.getTerms() != null
                  ? normalizePercent(detail1.getTerms().getPercent())
                  : null);
          throw new IllegalArgumentException(message);
        }
      }
  ...
  private boolean isDuplicateOrderDiscountPercent(CouponDetail detail1, CouponDetail detail2) {
    ...
    if (detail1.getTargetType() != TargetType.TICKET
        || detail1.getBenefitType() != BenefitType.DISCOUNT_PERCENT
        || detail2.getTargetType() != TargetType.TICKET
        || detail2.getBenefitType() != BenefitType.DISCOUNT_PERCENT) {
      return false;
    }
    ...
    BigDecimal percent2 =
        detail2.getTerms() != null ? normalizePercent(detail2.getTerms().getPercent()) : null;
  ...
  /**
   * Checks if two BigDecimal values are exactly equal (including null handling) null != 0, null ==
   * null, 10.00 == 10.0
   *
