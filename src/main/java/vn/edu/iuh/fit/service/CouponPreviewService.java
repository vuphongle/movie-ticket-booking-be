...
public class CouponPreviewService {
  ...
  public CouponPreviewResponse previewCoupon(Integer couponId, CouponPreviewRequest request) {
    ...
    List<CouponDetail> enabledDetails =
        couponDetailRepository.findByCouponIdAndEnabledTrueOrderByIdAsc(couponId);
  ...
  public CouponPreviewResponse previewAllCouponDetailDisplay(CouponPreviewRequest request) {
    ...
    // Lấy các details đã enabled và sắp xếp theo ID
    List<CouponDetail> enabledDetails = couponDetailRepository.findAllValidDisplayCouponDetails();
  ...
  public CouponApplyResponse applyCoupon(CouponApplyRequest request) {
    ...
    String validationMessage = validateCouponAvailability(coupon);
    if (validationMessage != null) {
      return CouponApplyResponse.builder().status("failed").errorMessage(validationMessage).build();
    }
    ...
    List<Integer> appliedDetailIds =
        previewResult.getDetailResults().stream()
  ...
  public CouponApplyResponse applyCouponDisplay(CouponApplyRequest request) {
    ...
    Coupon coupon =
        couponRepository
            .findById(couponDetail.getCouponId())
            .orElseThrow(() -> new ResourceNotFoundException("Coupon không tồn tại"));
  ...
  private CouponPreviewResponse calculateCouponApplication(
      List<CouponDetail> details, CouponPreviewRequest request) {
    ...
    for (CouponDetail detail : details) {
      CouponPreviewResponse.DetailApplicationResult result =
          applyDetailToCart(detail, request, originalOrderTotal);
      ...
      if (result.getApplied()) {
        ...
        if (detail.getBenefitType() == BenefitType.FREE_PRODUCT && detail.getTerms() != null) {
          AdditionalService giftService =
              additionalServiceRepository
                  .findById(detail.getTerms().getGiftServiceId())
                  .orElse(null);
          assert giftService != null;
          gifts.add(
              CouponPreviewResponse.GiftItem.builder()
  ...
  private CouponPreviewResponse.DetailApplicationResult applyToOrder(
      CouponDetail detail, CouponPreviewRequest request) {
    // Tính tổng tiền của toàn bộ đơn hàng
    BigDecimal orderTotal = calculateOriginalOrderTotal(request);
    int totalQuantity =
        request.getTickets().stream().mapToInt(CouponPreviewRequest.TicketItem::getQty).sum()
  ...
  private CouponPreviewResponse.DetailApplicationResult applyToSeatType(
      CouponDetail detail, CouponPreviewRequest request) {
  ...
  private CouponPreviewResponse.DetailApplicationResult applyToService(
      CouponDetail detail, CouponPreviewRequest request) {
    ...
    int totalQty =
        matchingServices.stream().mapToInt(CouponPreviewRequest.ServiceItem::getQty).sum();
    BigDecimal totalValue =
  ...
  private BigDecimal calculateDiscount(CouponDetail detail, BigDecimal baseValue, int quantity) {
    ...
    if (terms.getLimitQuantityApplied() != null && quantity > terms.getLimitQuantityApplied()) {
      ...
      BigDecimal ratio =
          BigDecimal.valueOf(terms.getLimitQuantityApplied())
