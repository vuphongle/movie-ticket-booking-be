  ...
  public Genre updateGenre(Integer id, UpsertGenreRequest genre) {
    Genre existingGenre =
        genreRepository
            .findById(id)
            .orElseThrow(
                () -> new ResourceNotFoundException("Không tìm thấy thể loại có id = " + id));

    // Kiểm tra tên thể loại đã tồn tại hay chưa. Nếu đã tồn tại và không phải là thể loại cần
    // update thì throw exception
    if (genreRepository.findByName(genre.getName()).isPresent()
        && !Objects.equals(existingGenre.getName(), genre.getName())) {
      throw new BadRequestException("Thể loại đã tồn tại");
    }
  ...
  public void deleteGenre(Integer id) {
    ...
    if (count > 0) {
      throw new BadRequestException(
          "Không thể xóa thể loại này vì có " + count + " phim thuộc thể loại này");
    }
