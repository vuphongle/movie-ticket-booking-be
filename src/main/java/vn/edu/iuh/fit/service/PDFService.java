...
public class PDFService {
    ...
    public String generateOrderPdfToS3(Order order, String qrCodeBase64) throws Exception {
        ...
        try (InputStream regularFont = new ClassPathResource("fonts/NotoSans-Regular.ttf").getInputStream();
             InputStream boldFont = new ClassPathResource("fonts/NotoSans-Bold.ttf").getInputStream()) {

            builder.useFont(() -> regularFont, "Noto Sans", 400,
                    PdfRendererBuilder.FontStyle.NORMAL, true);
            builder.useFont(() -> boldFont, "Noto Sans", 700,
                    PdfRendererBuilder.FontStyle.NORMAL, true);
    ...
    private String buildHtml(Order order, String qrCodeBase64, Map<String, Object> couponData) {
        String qrImg = (qrCodeBase64 != null && !qrCodeBase64.isEmpty())
        ...
        int totalTicketPrice = order.getTicketItems()
                .stream()
                .mapToInt(OrderTicketItem::getPrice)
                .sum();

        int totalServicePrice = order.getServiceItems()
                .stream()
        ...
        sb.append("<p>Xin chào <strong>")
                ...
                .append(" (").append(StringEscapeUtils.escapeXml10(userPhone)).append(")")
                .append("</strong>,</p>")
                ...
                .append(" / ").append(StringEscapeUtils.escapeXml10(auditoriumName))
                .append("</strong></li>")
                .append("<li>Trạng thái: <strong>")
                .append(status.equals("CONFIRMED") ? "Đã xác nhận" :
        ...
        if (services != null && !services.isEmpty()) {
            ...
            for (OrderServiceItem s : services) {
                sb.append("<tr><td>")
                        ...
                        .append("</td><td>").append(s.getQuantity())
                        .append("</td><td>").append(String.format("%,d", s.getPrice() * s.getQuantity()))
                        .append("</td></tr>");
            }
            sb.append("</tbody></table>");
        } else {
        ...
        if (couponData != null && couponData.containsKey("coupons")) {
            sb.append("<h4>Mã giảm giá / Quà tặng:</h4>")
                    .append("<table><thead><tr><th>Mã</th><th>Loại</th><th>Giảm (VNĐ)</th><th>Quà tặng</th></tr></thead><tbody>");
