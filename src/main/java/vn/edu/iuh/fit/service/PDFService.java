...
public class PDFService {
    ...
    private String buildHtml(Order order, String qrCodeBase64, Map<String, Object> couponData) {
        ...
        sb.append("<!DOCTYPE html>")
                .append("<html xmlns='http://www.w3.org/1999/xhtml'><head><meta charset='UTF-8'/><title>Order PDF</title>")
                .append("<style>")
                .append("@font-face {font-family:'Noto Sans'; src: url('fonts/NotoSans-Regular.ttf') format('truetype'); font-weight: normal;}")
                .append("@font-face {font-family:'Noto Sans'; src: url('fonts/NotoSans-Bold.ttf') format('truetype'); font-weight: bold;}")
                .append("@page { size: 90mm 220mm; margin: 5mm; }")
                .append("body { font-family:'Noto Sans', sans-serif; font-size:10pt; margin:0; padding:0; }")
                .append(".container { width:100%; padding:0; }")
                .append(".header,h2{text-align:center; margin:0;} .content h3{text-align:center; margin:5px 0;}")
        ...
        sb.append("<p>Xin chào <strong>").append(StringEscapeUtils.escapeXml10(userName))
                .append(" (").append(StringEscapeUtils.escapeXml10(userPhone)).append(")</strong>,</p>")
                .append("<p>Thông tin đơn hàng của bạn:</p>")
                .append("<ul>")
                .append("<li>Mã đơn hàng: <strong>").append(order.getId()).append("</strong></li>")
                .append("<li>Phim: <strong>").append(StringEscapeUtils.escapeXml10(movieTitle)).append("</strong></li>")
                .append("<li>Ngày/giờ chiếu: <strong>").append(showDate).append(" | ")
                .append(startTime).append(" - ").append(endTime).append("</strong></li>")
                .append("<li>Rạp / Phòng: <strong>").append(StringEscapeUtils.escapeXml10(cinemaName))
                .append(" / ").append(StringEscapeUtils.escapeXml10(auditoriumName)).append("</strong></li>")
                .append("<li>Trạng thái: <strong>")
                .append(status.equals("CONFIRMED") ? "Đã xác nhận" : (status.equals("CANCELLED") ? "Đã hủy" : status))
                .append("</strong></li></ul>");

        // Danh sách ghế
        sb.append("<h4>Danh sách ghế:</h4><table><thead><tr><th>Ghế</th><th>Loại</th><th>Giá (VNĐ)</th></tr></thead><tbody>");
        for (OrderTicketItem ticket : order.getTicketItems()) {
            sb.append("<tr><td>").append(StringEscapeUtils.escapeXml10(ticket.getSeat().getCode())).append("</td>")
                    .append("<td>").append(StringEscapeUtils.escapeXml10(String.valueOf(ticket.getSeat().getType()))).append("</td>")
                    .append("<td>").append(String.format("%,d", ticket.getPrice())).append("</td></tr>");
        }
        ...
        if (services != null && !services.isEmpty()) {
            sb.append("<h4>Combo/Dịch vụ:</h4><table><thead><tr><th>Sản phẩm</th><th>Số lượng</th><th>Giá (VNĐ)</th></tr></thead><tbody>");
            for (OrderServiceItem s : services) {
                sb.append("<tr><td>").append(StringEscapeUtils.escapeXml10(s.getAdditionalService().getName())).append("</td>")
                        .append("<td>").append(s.getQuantity()).append("</td>")
                        .append("<td>").append(String.format("%,d", s.getPrice() * s.getQuantity())).append("</td></tr>");
            }
            sb.append("</tbody></table>");
        } else {
        ...
        if (couponData.containsKey("coupons")) {
            ...
            sb.append("<table border='1' cellspacing='0' cellpadding='4' style='border-collapse:collapse;width:100%;'><thead><tr><th>Mã</th><th>Loại</th><th>Giảm</th><th>Quà tặng</th></tr></thead><tbody>");
            ...
            for (CouponDetailRequest c : coupons) {
                ...
                hasValidCoupon = true;
                sb.append("<tr>")
                        .append("<td style='word-break:break-all;'>").append(StringEscapeUtils.escapeXml10(displayCode)).append("</td>")
                        .append("<td>").append(StringEscapeUtils.escapeXml10(c.getType())).append("</td>")
                        .append("<td>").append(StringEscapeUtils.escapeXml10(String.valueOf(c.getDiscount()))).append("</td>")
                        .append("<td>");
            ...
            if (!hasValidCoupon) {
                sb.append("<tr><td colspan='4' style='text-align:center;'>Không áp dụng mã khuyến mãi nào</td></tr>");
            }
