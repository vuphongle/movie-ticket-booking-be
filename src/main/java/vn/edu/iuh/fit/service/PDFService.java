...
public class PDFService {
  ...
  private String buildHtml(Order order, String qrCodeBase64) {
    ...
    String cinemaName =
        StringEscapeUtils.escapeXml10(order.getShowtime().getAuditorium().getCinema().getName());
    String auditoriumName =
        StringEscapeUtils.escapeXml10(order.getShowtime().getAuditorium().getName());
    ...
    if (!qrImg.isEmpty()) {
      sb.append("<div class='qr-code'><img src='")
          .append(qrImg)
          .append("' width='200' height='200'/></div>");
    }
    sb.append("<p>Vui lòng giữ QR code này để xuất trình khi đến rạp.</p>")
        .append("</div>")
        .append("<p>Xin chào <strong>")
        .append(userName)
        .append(" (")
        .append(userPhone)
        .append(")</strong>,</p>")
    ...
    sb.append("<h4>Danh sách ghế:</h4>")
        .append(
            "<table><thead><tr><th>Ghế</th><th>Loại</th><th>Giá (VNĐ)</th></tr></thead><tbody>");
    ...
    if (services != null && !services.isEmpty()) {
      sb.append("<h4>Combo/Dịch vụ:</h4>")
          .append(
              "<table><thead><tr><th>Sản phẩm</th><th>Số lượng</th><th>Giá (VNĐ)</th></tr></thead><tbody>");
