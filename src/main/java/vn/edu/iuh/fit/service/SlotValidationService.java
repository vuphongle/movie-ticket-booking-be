...
public class SlotValidationService {
    ...
    private static final List<String> SLOT_START_TIMES = Arrays.asList(
        "08:00", "10:30", "13:00", "15:30", "18:00", "20:30"
    );
    
    private static final List<String> SLOT_END_TIMES = Arrays.asList(
    ...
    public void validateSlotBasedShowtime(String startTime, String endTime, int movieRuntimeMin) {
        log.info("Validating slot-based showtime: start={}, end={}, runtime={}", startTime, endTime, movieRuntimeMin);
        
        // 1. Validate startTime matches slot boundaries
        int slotIndexStart = getSlotIndexByStartTime(startTime);
        if (slotIndexStart == -1) {
            throw new InvalidSlotException("Giờ bắt đầu không khớp với ca chiếu hợp lệ. Vui lòng chọn: " + String.join(", ", SLOT_START_TIMES));
        }
        ...
        log.info("Movie runtime: {}min, effective: {}min, spans: {}", movieRuntimeMin, effectiveRuntime, spans);
        
        // 3. Check if movie is too long
        if (spans > MAX_SLOT_SPANS) {
            throw new MovieTooLongException("Phim quá dài, cần " + spans + " ca liên tiếp. Hệ thống chỉ hỗ trợ tối đa " + MAX_SLOT_SPANS + " ca.");
        }
        
        // 4. Check span overflow
        int endSlotIndex = slotIndexStart + spans - 1;
        if (endSlotIndex >= SLOT_END_TIMES.size()) {
            throw new SpanOverflowException("Suất chiếu vượt quá khung giờ cuối ngày. Vui lòng chọn ca sớm hơn.");
        }
        ...
        if (!endTime.equals(expectedEndTime)) {
            throw new InvalidSlotException("Giờ kết thúc không khớp với ca chiếu. Mong đợi: " + expectedEndTime + ", nhận được: " + endTime);
        }
        
        log.info("Slot validation passed: slot {} to {} (spans={})", slotIndexStart + 1, endSlotIndex + 1, spans);
    }
