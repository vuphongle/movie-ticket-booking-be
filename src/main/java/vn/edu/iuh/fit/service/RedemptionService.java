...
public class RedemptionService {
  ...
  private static final Set<String> processedRedemptions =
      new HashSet<>(); // Simple in-memory cache - should use Redis in production
  ...
  public void confirmRedemption(RedemptionConfirmRequest request) {
    ...
    if (processedRedemptions.contains(redemptionKey)) {
      log.warn(
          "Redemption already processed for order {} and coupon {}",
          request.getOrderId(),
    ...
    for (Integer detailId : request.getAppliedDetailIds()) {
      CouponDetail detail =
          couponDetailRepository
              .findById(detailId)
              .orElseThrow(
                  () -> new ResourceNotFoundException("Coupon detail không tồn tại: " + detailId));
      ...
      if (terms == null) {
        ...
        terms = CouponDetailTerms.builder().couponDetail(detail).detailUsedCount(0).build();
      }
      ...
      log.info(
          "Increased used count for coupon detail {} to {}", detailId, terms.getDetailUsedCount());
    }

    // Đánh dấu đã xử lý
    processedRedemptions.add(redemptionKey);
    log.info(
        "Confirmed redemption for order {} and coupon {}",
        request.getOrderId(),
        request.getCouponCode());
  }
  ...
  public void revertRedemption(RedemptionConfirmRequest request) {
    ...
    if (!processedRedemptions.contains(redemptionKey)) {
      log.warn(
          "Redemption not found for order {} and coupon {}",
          request.getOrderId(),
          request.getCouponCode());
    ...
    for (Integer detailId : request.getAppliedDetailIds()) {
      ...
      if (terms != null) {
        ...
        if (terms.getDetailUsedCount() > 0) {
          ...
          log.info(
              "Decreased used count for coupon detail {} to {}",
              detailId,
              terms.getDetailUsedCount());
        }
      }
    }
    ...
    log.info(
        "Reverted redemption for order {} and coupon {}",
        request.getOrderId(),
        request.getCouponCode());
  }
