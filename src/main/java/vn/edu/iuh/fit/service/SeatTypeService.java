...
public class SeatTypeService {
    ...
    private static final Map<Integer, SeatType> ID_TO_SEAT_TYPE = Map.of(
        1, SeatType.NORMAL,
        2, SeatType.VIP,
        3, SeatType.COUPLE
    );
    
    private static final Map<SeatType, Integer> SEAT_TYPE_TO_ID = Map.of(
        SeatType.NORMAL, 1,
        SeatType.VIP, 2,
        SeatType.COUPLE, 3
    );
    ...
    public List<SeatTypeOption> getAllSeatTypeOptions() {
        return Arrays.stream(SeatType.values())
            .map(seatType -> new SeatTypeOption(
                getIdBySeatType(seatType),
                seatType.name(),
