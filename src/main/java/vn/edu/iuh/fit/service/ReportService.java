...
public class ReportService {
  ...
  public byte[] exportRevenueByMovieId(
      Integer movieId, String startDate, String endDate, String userEmail) {
    try (Workbook workbook = new XSSFWorkbook()) {
      Sheet sheet = workbook.createSheet("Doanh thu theo phim");
      sheet.setDisplayGridlines(false);
      ...
      // Get data
      List<MovieCinemaRevenueDto> movieCinemaRevenues =
          dashboardService.getRevenueByMovieId(movieId, startDate, endDate);

      // Get movie name from first record (if available)
      String movieName =
          movieCinemaRevenues.isEmpty() ? "N/A" : movieCinemaRevenues.get(0).getMovieName();
      String movieCode =
          movieCinemaRevenues.isEmpty() ? "N/A" : movieCinemaRevenues.get(0).getMovieCode();
      ...
      Row titleRow = sheet.createRow(3);
      Cell titleCell = titleRow.createCell(0);
      titleCell.setCellValue("DOANH THU THEO PHIM: " + movieCode + " - " + movieName);
      titleCell.setCellStyle(titleStyle);
      ...
      for (MovieCinemaRevenueDto dto : movieCinemaRevenues) {
        Row row = sheet.createRow(rowNum++);
  ...
  public byte[] exportRevenueByCinemaId(
      Integer cinemaId, String startDate, String endDate, String userEmail) {
    try (Workbook workbook = new XSSFWorkbook()) {
      Sheet sheet = workbook.createSheet("Doanh thu theo rạp");
      sheet.setDisplayGridlines(false);
      ...
      // Get data
      List<CinemaMovieRevenueDto> cinemaMovieRevenues =
          dashboardService.getRevenueByCinemaId(cinemaId, startDate, endDate);

      // Get cinema name from first record (if available)
      String cinemaName =
          cinemaMovieRevenues.isEmpty() ? "N/A" : cinemaMovieRevenues.get(0).getCinemaName();
      String cinemaCode =
          cinemaMovieRevenues.isEmpty() ? "N/A" : cinemaMovieRevenues.get(0).getCinemaCode();
      ...
      Row titleRow = sheet.createRow(3);
      Cell titleCell = titleRow.createCell(0);
      titleCell.setCellValue("DOANH THU THEO RẠP: " + cinemaCode + " - " + cinemaName);
      titleCell.setCellStyle(titleStyle);
      ...
      for (CinemaMovieRevenueDto dto : cinemaMovieRevenues) {
        Row row = sheet.createRow(rowNum++);
