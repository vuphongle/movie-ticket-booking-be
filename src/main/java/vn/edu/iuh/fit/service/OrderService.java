...
public class OrderService {
    ...
    @Autowired
    private CouponDetailTermsRepository couponDetailTermRepository;
    ...
    @Transactional
    public PaymentResponse createOrder(CreateOrderRequest request, String paymentMethod) throws JsonProcessingException {
        ...
        Order order = Order.builder()
                ...
                .discount(request.getDiscounts().getTotalDiscount() != null ? request.getDiscounts().getTotalDiscount() : 0)
        ...
        log.info("Order created with ID: {}, Total Price: {}, Discount: {}",
                savedOrder.getId(), savedOrder.getTotalPrice(), request.getDiscounts().getTotalDiscount());
        String paymentUrl;
        ...
        if ("VNPAY".equalsIgnoreCase(paymentMethod)) {
            ...
            paymentUrl = payOSService.createOrder(savedOrder.getTotalPrice(), String.valueOf(savedOrder.getId()), returnUrl, expireSeconds);
        } else {
        ...
        if (status == OrderStatus.CONFIRMED) {
            ...
            if (order.getDiscount() > 0 && order.getRequestSnapshot() != null) {
                try {
                    ...
                    CreateOrderRequest originalRequest = objectMapper.readValue(
                            order.getRequestSnapshot(),
                            CreateOrderRequest.class
                    );

                    log.info("Original request discounts: {}", originalRequest.getDiscounts().getCoupons());

                    if (originalRequest.getDiscounts() != null && originalRequest.getDiscounts().getCoupons() != null) {
                        for (CouponDetailRequest couponRequest : originalRequest.getDiscounts().getCoupons()) {
                            if (couponRequest.getDetailId() != null) {
                                CouponDetailTerms term = couponDetailTermRepository.findById(couponRequest.getDetailId())
                                        .orElseThrow(() -> new ResourceNotFoundException(
                                                "Không tìm thấy điều kiện coupon với id " + couponRequest.getDetailId()));
                                // Tăng số lượng đã dùng
                                log.info("Cập nhật coupon detail id {}: current used count = {}, increment by 1",
                                        term.getId(), term.getDetailUsedCount());
            ...
            for (OrderServiceItem serviceItem : order.getServiceItems()) {
                ...
                if (additionalService.getType() == AdditionalServiceType.COMBO) {
                    List<AdditionalServiceItem> items = additionalServiceItemRepository
                            .findByAdditionalServiceId(additionalService.getId());
                    ...
                    for (AdditionalServiceItem item : items) {
                        ...
                        if (product.getQuantity() != null) {
                            ...
                            log.info("Cập nhật kho sản phẩm id {}: trừ {} => new stock = {}",
                                    product.getId(), totalUsed, product.getQuantity());
                        }
                    }
                } else if (additionalService.getType() == AdditionalServiceType.SINGLE) {
                    if (additionalService.getProductId() != null) {
                        Product product = productRepository.findById(additionalService.getProductId())
                                .orElseThrow(() -> new ResourceNotFoundException(
                        ...
                        if (product.getQuantity() != null) {
                            ...
                            log.info("Cập nhật kho sản phẩm id {} (SINGLE): trừ {} => new stock = {}",
                                    product.getId(), orderedQty, product.getQuantity());
                        }
                    }
                }
            }
            ...
            if (order.getRequestSnapshot() != null) {
                try {
                    ObjectMapper objectMapper = new ObjectMapper();
                    CreateOrderRequest request = objectMapper.readValue(order.getRequestSnapshot(), CreateOrderRequest.class);
