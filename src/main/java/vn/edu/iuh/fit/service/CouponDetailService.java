...
public class CouponDetailService {
  ...
  public List<CouponDetailResponse> getCouponDetails(Integer couponId) {
    ...
    return details.stream().map(this::buildCouponDetailResponse).collect(Collectors.toList());
  }
  ...
  public CouponDetail getCouponDetailById(Integer detailId) {
    return couponDetailRepository
        .findById(detailId)
        .orElseThrow(() -> new ResourceNotFoundException("Coupon detail không tồn tại"));
  }

  @Transactional
  public CouponDetailResponse createCouponDetail(
      Integer couponId, UpsertCouponDetailRequest request) {
    Coupon coupon =
        couponRepository
            .findById(couponId)
    ...
    if (request.getTargetType() == TargetType.TICKET
        && request.getBenefitType() == BenefitType.DISCOUNT_PERCENT) {
      // Create temp detail with explicit fields for validation
      CouponDetail tempDetail =
          CouponDetail.builder()
      ...
      if (savedDetail.getTerms() != null) {
        CouponDetailTerms tempTerms =
            CouponDetailTerms.builder().percent(savedDetail.getTerms().getPercent()).build();
  ...
  @Transactional
  public CouponDetailResponse updateCouponDetail(
      Integer detailId, UpsertCouponDetailRequest request) {
    CouponDetail detail =
  ...
  public CouponDetailResponse duplicateCouponDetail(Integer detailId) {
    ...
    if (originalDetail.getTerms() != null) {
      ...
      CouponDetailTerms duplicatedTerms =
          CouponDetailTerms.builder()
              .couponDetail(savedDetail)
  ...
  private void validateCouponDetailRequest(UpsertCouponDetailRequest request) {
    ...
    if (request.getTerms() != null) {
      switch (request.getBenefitType()) {
        ...
        case DISCOUNT_AMOUNT:
          if (request.getTerms().getAmount() == null
              || request.getTerms().getAmount().compareTo(BigDecimal.ZERO) <= 0) {
            throw new BadRequestException(ValidationMessages.INVALID_AMOUNT_VALUE);
          }
          break;
        case FREE_PRODUCT:
          if (request.getTerms().getGiftServiceId() == null
              || request.getTerms().getGiftQuantity() == null
    ...
    if (request.getTerms() != null
        && request.getTerms().getLimitQuantityApplied() != null
        && request.getTerms().getLimitQuantityApplied() < 0) {
  ...
  private CouponDetailResponse buildCouponDetailResponse(CouponDetail detail) {
    CouponDetailResponse.CouponDetailResponseBuilder builder =
