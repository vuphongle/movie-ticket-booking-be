...
public class AdditionalServices {
  ...
  public AdditionalService updateAdditionalService(
      Integer id, UpsertAdditionalServiceRequest request) {
    ...
    if (request.getType() == AdditionalServiceType.SINGLE) {
      ...
      if (request.getDefaultQuantity() == null || request.getDefaultQuantity() <= 0) {
        throw new IllegalArgumentException(
            "Default quantity must be greater than 0 for SINGLE type service");
      }
  ...
  private void createAdditionalServiceItems(
      AdditionalService additionalService,
