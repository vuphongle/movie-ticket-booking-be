...
public interface AdditionalServiceItemRepository
    extends JpaRepository<AdditionalServiceItem, Integer> {
  ...
  @Query(
      "SELECT asi FROM AdditionalServiceItem asi "
          + "JOIN FETCH asi.product p "
          + "WHERE asi.additionalService.id = :serviceId")
  List<AdditionalServiceItem> findByAdditionalServiceIdWithProduct(
  ...
  @Query(
      "SELECT COUNT(asi) > 0 FROM AdditionalServiceItem asi "
