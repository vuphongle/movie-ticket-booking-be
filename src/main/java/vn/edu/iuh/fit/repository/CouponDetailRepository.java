...
public interface CouponDetailRepository extends JpaRepository<CouponDetail, Integer> {
  ...
  @Query(
      "SELECT cd FROM CouponDetail cd WHERE cd.coupon.id = :couponId AND cd.enabled = true ORDER BY cd.id ASC")
  ...
  @Query(
      "SELECT COUNT(cd) FROM CouponDetail cd WHERE cd.coupon.id = :couponId AND cd.enabled = true")
  ...
  @Modifying
  @Transactional
  @Query("UPDATE CouponDetail cd SET cd.enabled = false WHERE cd.coupon.id = :couponId")
  void disableAllByCouponId(@Param("couponId") Integer couponId);
}
